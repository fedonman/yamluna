...
pytest.importorskip('yamluna._yamluna', reason='extension not built yet: maturin develop --uv')
...
def _rt(lib: Any) -> Any:
    """Return a round-trip `YAML` on the ordinary recipe, identical for both libraries."""
    yaml = lib.YAML()
...
def load(text: str, lib: Any = yamluna) -> Any:
    """Load the first document of `text` through `lib`, which is yamluna by default."""
    return _rt(lib).load(text)


def dump(node: Any, lib: Any = yamluna) -> str:
    """Dump one document through `lib` and returns the text it wrote."""
    buf = io.StringIO()
...
def mutated(text: str, mutate: Any, lib: Any = yamluna) -> str:
    """Load, mutates, then dumps: the operation every test here measures.
...
def attach(text: str) -> dict[str, tuple[tuple[str, ...], str | None]]:
    """Read which comment describes which element, out of the emitted text alone.
...
def surviving(
    ...
    *,
    gone: AbstractSet[str] = frozenset(),
    added: AbstractSet[str] = frozenset(),
) -> dict[str, tuple[tuple[str, ...], str | None]]:
    """Return the attachment map a mutation must produce: the invariant, spelled out once.
...
def test_a3_deleting_the_last_item_leaves_no_comment_dangling() -> None:
    assert mutated(SEQ, lambda s: s.__delitem__(2)) == ('# about one\n- one\n# about two\n- two\n')
...
def test_map_popitem_takes_the_last_entry_s_comments_and_leaves_the_trailing_one() -> None:
    assert attach(mutated(EVERY, lambda m: m.popitem())) == surviving(EVERY_BASE, gone={'gamma'})
...
def test_map_update_keeps_existing_comments_and_adds_new_keys_bare() -> None:
    assert attach(mutated(EVERY, lambda m: m.update({'alpha': 11, 'delta': 4}))) == surviving(
        EVERY_BASE, added={'delta'}
    )
...
def test_no_mutation_invents_a_comment(name: str) -> None:
    ...
    after = {
        line.strip() for line in mutated(src, mutate).splitlines() if line.strip().startswith('#')
    }
    assert after <= before
