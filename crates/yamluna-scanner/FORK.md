...
`yamluna-core` does to fill `Node::raw` (§2), so this would have made a byte-exact round
...
- The `Input` trait was **not** changed to carry byte offsets (§1.5 forbids it);
...
which is what keeps the 402 conformance cases green.
...
`fetch_flow_entry` build their spans after `skip_ws_to_eol`. Unlike B3 this is harmless, since
those spans only feed a collection's *start* position and collections have no `raw`, so it was
left as upstream has it. The comment's own span is exact either way.
...
(`src/parser.rs:441`) returns it. It is set while a document's directives are consumed, that is,
...
driving the parser as an iterator (which `yamluna-core` does, per §2.3) reads it on
`DocumentStart`. **A receiver driven by `Parser::load` cannot see it**, which is the one cost of
this choice.
