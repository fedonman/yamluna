...
def corpus_bytes(corpus_path: Path) -> bytes:
    """Return the corpus file's exact bytes, which is what byte-identity is measured against.
...
def corpus_text(corpus_path: Path) -> str:
    """Return the corpus file decoded as UTF-8, with the BOM and CRLF left in place.
...
def yamluna() -> Any:
    """Return the yamluna package, which imports without the Rust extension.
...
def yamluna_roundtrip(yamluna: Any) -> Callable[[str], str]:
    """Return the acceptance operation: text in, load, dump, text out.
...
def ruamel_roundtrip() -> Callable[[str], str]:
    """Return the ruamel.yaml oracle, for tests that compare the two implementations directly.
