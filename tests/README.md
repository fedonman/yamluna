...
| `flow-nesting`                | **no** | yes     |  |
| `key-complex`                 | **no** | yes     |  |
| `key-duplicate`               | **no** | **no**  | 18 lines -> 15; line 6: `a: 1` -> `a: 3`; warns DuplicateKeyFutureWarning |
...
| `text-bom`                    | **no** | yes     |  |
| `text-crlf`                   | **no** | yes     |  |
| `text-tabs`                   | **no** | **no**  | line 19: `...w_before_comma: [a<TAB>, b]` -> `...w_before_comma: [a,  b]` |
| `text-unicode`                | **no** | yes     |  |

### Known gaps

The three files yamluna does not yet round-trip, one line each. Every one is pinned by a
guard list that fails the suite if it starts passing, so a future fix is noticed rather than
silently absorbed:

| corpus file | why it does not round-trip | pinned by |
|---|---|---|
| `flow-forms` | the record classes have no slot for `Node.flow_comma` / `flow_end` / `flow_bare_key`, so `[ 1 , 2 ]`, a trailing comma and `{a: , b}` lose their punctuation crossing the FFI — the core keeps all three and the pure-Rust round trip reproduces the file | `KNOWN_RECORD_GAPS` (`test_bindings.py`), `KNOWN_LOSSES` (`test_roundtrip.py`) |
| `key-duplicate` | `CommentedMap` is a `dict`, so two entries with equal keys collapse into one — and the default `allow_duplicate_keys=False` refuses the file before that | `KNOWN_LOSSES` (`test_roundtrip.py`) |
| `text-tabs` | the model records lexemes and positions but not the white space *between* two lexemes, so the emitter reaches each recorded column with `Writer::pad_to`, which writes spaces, and `[a<TAB>, b]` comes back `[a,  b]` | `KNOWN_FAILURES` (`crates/yamluna-core/tests/roundtrip.rs`), `KNOWN_RECORD_GAPS`, `KNOWN_LOSSES` |

Only `key-duplicate` is a wall: it is the price of DESIGN 4.1, where the types subclass the
builtins so that `isinstance(x, dict)`, `json.dumps`, `copy.deepcopy` and `==` work for free.
The other two are additive — three record slots, and one inter-token white-space run in the
model — and neither needs a design change.

Each layer loses exactly one more fact than the one below it, which is what the numbers are
for. Measured, not inferred:

| layer | round-trips | what it adds |
|---|---|---|
| `yamluna-core` (Rust `parse` -> `emit`) | **40 of 41** | `text-tabs` |
| the FFI records (`emit(parse(text))` through `_record`) | **39 of 41** | + `flow-forms` |
| `YAML().dump(YAML().load(text))` | **38 of 41** | + `key-duplicate` |
...
| `key-duplicate`               | **no** | **no**  | re-indented block sequences; 18 lines -> 15 |
| `scalar-binary`               | **no** | yes     | 11 lines -> 12; line 5: `...eXvPz7Y6OjuDg4J+fn5` -> `...eXvPz7Y6OjuDg4J+fn5OTk6enp56enmlp` |
...
directive, marker, BOM, CRLF and comment, including in the two files that are nothing but
comments and empty documents, never moves an end-of-line comment, never re-indents or
refolds an untouched node, and never re-spells a scalar it did not touch. Group 5 is done
for anchors and aliases, which stay aliases rather than being cloned; duplicate keys are
reported rather than raised on by default, but still cannot be *represented* while
`CommentedMap` is a `dict` — the one remaining wall, and the only one of the three known
gaps above that a design change would be needed to move.
